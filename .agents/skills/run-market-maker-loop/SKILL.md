...
Spawn three workers using `model: "gpt-5.6-luna"`, `reasoning_effort: "medium"`, and `fork_turns: "none"`. Give each worker its complete worktree path, hypothesis, implementation plan, repository constraints, and checks. Workers may change only the single target core method plus `MarketMaker` helpers, required imports, and `on_trade`. `on_trade` is bookkeeping rather than strategy and is exempt from the one-target freeze, so an accumulator and the code consuming it land in the same generation; the grader hands it the price and counterparty of every executed trade. Its signature stays frozen and it must keep calling `self.position.add_option_quantity`, which the validator enforces. `on_step_advance`, `__init__`, `name`, `price_option` and `price_option_from_parameters` remain frozen. They must validate against the selected parent with `validate-candidate.sh --target-method <method>`, compile with a temporary `PYTHONPYCACHEPREFIX`, inspect their diff, and must not run HackerRank or commit. Never put candidate-level or plural target-method declarations in the plan.
