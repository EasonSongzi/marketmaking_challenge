...
class MarketMaker:
    ...
    def respond_to_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:  # type: ignore[empty-body]
        ...
        if not hasattr(self, "_fok_counterparty_counts"):
            self._fok_counterparty_counts: dict[int, int] = {}
        prior_requests: int = self._fok_counterparty_counts.get(fok_order.counterparty_id, 0)
        self._fok_counterparty_counts[fok_order.counterparty_id] = prior_requests + 1
