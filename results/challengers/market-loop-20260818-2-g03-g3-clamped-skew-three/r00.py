...
class MarketMaker:
    ...
    def quote(self, option: BinaryOption, counterparty_id: int) -> Quote:  # type: ignore[empty-body]
        ...
        inventory: int = self.position.option_quantity_by_option_id[option.option_id]
        center_cents: int = fair_value_cents
        if inventory > 0:
            center_cents -= 3
        elif inventory < 0:
            center_cents += 3
        center_cents = min(max(center_cents, 0), 100)
        bid_price: float = max(center_cents - 3, 0) / 100
        offer_price: float = min(center_cents + 3, 100) / 100
        return Quote(
            bid_price=bid_price,
            bid_quantity=2,
