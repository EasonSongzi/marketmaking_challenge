...
class MarketMaker:
    ...
    def _fit_penalized_rate(
        ...
        for candidate_index in range(1001):
            ...
            score: float = (
                log_likelihood
                - 25.0 * (rate_up_probability - smoothed_up_probability) ** 2
                - 25.0 * (rate_down_probability - smoothed_down_probability) ** 2
                - 2.0 * (reversion_strength - moment_reversion_strength) ** 2
            )
            if score > best_score:
                best_score = score
                best_parameters = (
