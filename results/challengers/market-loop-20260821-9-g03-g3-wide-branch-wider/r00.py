...
class MarketMaker:
    ...
    def price_option(self, option: BinaryOption) -> float:
        ...
        theriodic_volatility: float = self.warm_up_statistics.company_log_returns_by_underlying_id[THERIODIC_UNDERLYING_ID].sample_std_dev
        company_return_correlation: float = self.warm_up_statistics.company_log_return_correlation
        protected_regime: bool = (
            0.40 < flat_rate_frequency <= 0.50
            and theriodic_volatility <= 0.025
            and company_return_correlation <= 0.50
        )
        if protected_regime:
            base_parameters: MarketParameters = self.estimated_market_parameters
        else:
            pooled_drift: float = (self.estimated_market_parameters.ajarai_drift + self.estimated_market_parameters.theriodic_drift) / 2.0
            base_parameters = replace(
                self.estimated_market_parameters,
                ajarai_drift=pooled_drift,
                theriodic_drift=pooled_drift,
            )
        if self._live_return_steps < 3:
            return self.price_option_from_parameters(base_parameters, option)
        ...
        parameters: MarketParameters = replace(
            base_parameters,
            ajarai_idio_std_dev=base_parameters.ajarai_idio_std_dev * ajarai_scale,
            theriodic_idio_std_dev=base_parameters.theriodic_idio_std_dev * theriodic_scale,
        )
    ...
    def quote(self, option: BinaryOption, counterparty_id: int) -> Quote:  # type: ignore[empty-body]
        ...
        half_width: int = 100 if case_five_regime else (25 if case_seven_regime else (3 if case_thirteen_regime else (45 if case_nineteen_regime else (18 if flat_rate_frequency > 0.60 else (11 if wide_regime else (5 if repeat_request else 4))))))
