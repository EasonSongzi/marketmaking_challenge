...
class MarketMaker:
    ...
    def quote(self, option: BinaryOption, counterparty_id: int) -> Quote:  # type: ignore[empty-body]
        ...
        for active_option in self.active_option_state:
            quantity = self.position.option_quantity_by_option_id.get(active_option.option_id, 0)
            theoretical_value = self.price_option(active_option)
            if quantity > 0:
                long_reserve += quantity * theoretical_value
            elif quantity < 0:
                short_reserve += -quantity * (1.0 - theoretical_value)
        cash_floor = 0.75 * self.cash_balance
        available_capacity = self.cash_balance - cash_floor
        bid_quantity = (
            3
            if long_reserve + 3 * bid_price <= available_capacity or bid_price <= 0.25
            else 2
        )
        offer_quantity = (
            3
            if short_reserve + 3 * (1.0 - offer_price) <= available_capacity
