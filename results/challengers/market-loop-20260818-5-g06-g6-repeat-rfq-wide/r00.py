...
class MarketMaker:
    ...
    def quote(self, option: BinaryOption, counterparty_id: int) -> Quote:  # type: ignore[empty-body]
        quote_counts: dict[int, int] = getattr(self, "_quote_counts", {})
        request_count: int = quote_counts.get(counterparty_id, 0)
        quote_counts[counterparty_id] = request_count + 1
        self._quote_counts = quote_counts
        fair_value_cents: int = round(self.price_option(option) * 100)
        half_width: int = 4 if request_count == 0 else 5
