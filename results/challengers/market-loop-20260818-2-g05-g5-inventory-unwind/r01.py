...
class MarketMaker:
    ...
    def respond_to_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:  # type: ignore[empty-body]
        ...
        unwinds_position = (
            (position > 0 and fok_order.order_type == OrderType.BUY)
            or (position < 0 and fok_order.order_type == OrderType.SELL)
        )
        if unwinds_position and fok_order.quantity <= abs(position):
        ...
        if fok_order.order_type == OrderType.BUY:
            return (
                fok_order.price >= theoretical_value + 0.025
