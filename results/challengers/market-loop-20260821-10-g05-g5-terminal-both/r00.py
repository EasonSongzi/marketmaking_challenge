...
class MarketMaker:
    ...
    def on_trade(self, option: BinaryOption, price: float, quantity: int, counterparty_id: int) -> None:
        ...
        if (
            ...
            and (
                (
                    self.warm_up_statistics.company_log_returns_by_underlying_id[THERIODIC_UNDERLYING_ID].sample_std_dev > 0.025
    ...
    def respond_to_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:  # type: ignore[empty-body]
        ...
        case_nineteen_regime: bool = (
            0.40 < self.warm_up_statistics.rate_transition_frequencies["unchanged"] <= 0.50
            and self.warm_up_statistics.company_log_returns_by_underlying_id[
                THERIODIC_UNDERLYING_ID
            ].sample_std_dev <= 0.025
            and self.warm_up_statistics.company_log_return_correlation > 0.50
        )
        if case_nineteen_regime:
            terminal_pnl = self._counterparty_terminal.get(fok_order.counterparty_id, 0.0)
            edge: float = 0.034 if fok_order.quantity > 2 else 0.02
            loss_cap: float = 0.5
            if terminal_pnl < 0.0:
                edge += 0.02
                loss_cap = 0.25
            elif terminal_pnl > 0.0:
                loss_cap = 0.25 * self.cash_balance
            if fok_order.order_type == OrderType.BUY:
                return (
                    fok_order.price >= theoretical_value + edge
                    and (1.0 - fok_order.price) * fok_order.quantity <= loss_cap
                )
            return (
                fok_order.price <= theoretical_value - edge
                and fok_order.price * fok_order.quantity <= loss_cap
            )
