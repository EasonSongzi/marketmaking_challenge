...
class MarketMaker:
    ...
    def respond_to_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:  # type: ignore[empty-body]
        ...
        markout: float = self._counterparty_markout.get(fok_order.counterparty_id, 0.0)
        if markout < 0.0:
            edge += 0.02
            cap: float = 0.25
        elif markout > 0.0:
            cap = max(0.5, 0.02 * self.cash_balance)
        else:
            cap = 0.5
        if fok_order.order_type == OrderType.BUY:
            return (
                fok_order.price >= theoretical_value + edge
                and (1.0 - fok_order.price) * fok_order.quantity <= cap
            )
        return (
            fok_order.price <= theoretical_value - edge
            and fok_order.price * fok_order.quantity <= cap
        )
